...
rule trim_galore:
    ...
    params:
        "--gzip -q 30"
    shell:
        "{SIF_EXEC} trim_galore {params} -o {TRIM_DIR} {input}"
...
rule align:
    ...
    threads: 5
    shell:
        "{SIF_EXEC} bowtie2 {params} -p {threads} -U {input} 2> {output.report_txt} | {SIF_EXEC} samtools view -bS - > {output.bam}"
...
rule keep_quality_alignments:
    ...
    params:
        filter = " and ".join([
            "\"not (unmapped)",    # remove: pairs where either mate is unmapped
